...
#
#     def forward(self, x):
#         fore_weights = self.filter_module(x)  # batch, time, channel
#
...
#
#         score_base = self.softmax(score_base)  # batch, class
#         score_supp = self.softmax(score_supp)  # batch, class
#
...
class BaS_Net(nn.Module):
    def __init__(self, len_feature, num_classes, num_segments):
        ...
        self.hidden = nn.Sequential(
            nn.Conv1d(len_feature, 2048, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(),
        ...
        self.cas_module = nn.Conv1d(1024, num_classes + 1, kernel_size=1)
        self.dropout = nn.Dropout(0.7)

    def forward(self, x):
        x = self.hidden(x.permute(0, 2, 1))
        fore_weights = self.filter_module(x)
        x_supp = fore_weights * x
        cas_base = self.cas_module(self.dropout(x)).permute(0, 2, 1)
        cas_supp = self.cas_module(self.dropout(x_supp)).permute(0, 2, 1)
